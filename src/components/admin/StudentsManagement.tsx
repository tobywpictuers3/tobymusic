...
const StudentsManagement = () => {
  ...
  useEffect(() => {
    // Safe before Sep 1: the rollover guard performs no write until the date arrives.
  ...
  const storedYearRecord = editingStudent
    ? getStudentSchoolYearRecord(editingStudent.id, formSchoolYear)
    : undefined;
  ...
  const handleSaveStudent = () => {
    ...
    const effectiveAmount = studentForm.paymentType === 'annual'
      ? Math.max(0, annualBaseTarget - openingBalance)
      : 0;
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>תשלום שנתי מלא</TableHead>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <Input id="firstName" value={studentForm.firstName} onChange={(e) => setStudentForm({...studentForm, firstName: e.target.value})} placeholder="שם פרטי" />
              </div>
              <div>
                <Label htmlFor="lastName">שם משפחה *</Label>
                <Input id="lastName" value={studentForm.lastName} onChange={(e) => setStudentForm({...studentForm, lastName: e.target.value})} placeholder="שם משפחה" />
              </div>
              <div>
                <Label htmlFor="personalCode">קוד אישי (4 ספרות) *</Label>
                <Input id="personalCode" value={studentForm.personalCode} onChange={(e) => setStudentForm({...studentForm, personalCode: e.target.value})} placeholder="0000" maxLength={4} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="phone" value={studentForm.phone} onChange={(e) => setStudentForm({...studentForm, phone: e.target.value})} placeholder="מספר טלפון" />
              </div>
              <div>
                <Label htmlFor="email">אימייל</Label>
                <Input id="email" type="email" value={studentForm.email} onChange={(e) => setStudentForm({...studentForm, email: e.target.value})} placeholder="כתובת אימייל" />
              </div>
            </div>
            ...
            <div className="p-4 bg-primary/10 rounded-lg border border-primary/20">
              ...
              {studentForm.paymentType === 'per_lesson' && (
                <div className="mt-3">
                  ...
                  <NumberStepper id="lessonPrice" value={studentForm.lessonPrice} onValueChange={(n) => setStudentForm({...studentForm, lessonPrice: n})} step={10} min={0} placeholder="150" unit="₪" />
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="startDate" type="date" value={studentForm.startDate} onChange={(e) => setStudentForm({...studentForm, startDate: e.target.value})} />
              </div>
              <div>
                <Label htmlFor="startingLessonNumber">מספר שיעור התחלתי</Label>
                <NumberStepper id="startingLessonNumber" value={studentForm.startingLessonNumber} onValueChange={(n) => setStudentForm({...studentForm, startingLessonNumber: Math.max(1, n)})} step={1} min={1} />
              </div>
            </div>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <NumberStepper id="annualAmount" value={studentForm.annualAmount} onValueChange={(n) => setStudentForm({...studentForm, annualAmount: Math.max(0, n)})} step={100} min={0} unit="₪" />
                  </div>
                  <div>
                    <Label htmlFor="paymentMonths">מס' חודשי תשלום</Label>
                    <NumberStepper id="paymentMonths" value={studentForm.paymentMonths} onValueChange={(n) => setStudentForm({...studentForm, paymentMonths: Math.min(12, Math.max(1, n))})} step={1} min={1} max={12} />
                  </div>
                </div>

                <div className="space-y-2 rounded-lg border border-primary/20 bg-muted/50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="font-semibold">חישוב אוטומטי</span>
                    <Badge variant="outline">{getSchoolYearStartReasonLabel(inferredStartReason)}</Badge>
                  </div>
                  {inferredStartReason === 'midyear_join' ? (
                    <p className="text-sm">
                      התחלה בשיעור #{studentForm.startingLessonNumber} בגלל הצטרפות באמצע שנה → חיוב עבור <b>{billedLessonCount}</b> שיעורים בלבד מתוך 38.
                    </p>
                  ) : inferredStartReason === 'carryover_credit' ? (
                    <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                      התחלה בשיעור #{studentForm.startingLessonNumber} היא זכות מיספור משנה קודמת → החיוב נשאר עבור <b>38 שיעורים</b> מלאים.
                    </p>
                  ) : (
                    <p className="text-sm">שנה מלאה → חיוב עבור <b>38 שיעורים</b>.</p>
                  )}
                  <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                    <div>מחיר מלא<br/><b>{money(studentForm.annualAmount)}</b></div>
                    <div>שיעורים לחיוב<br/><b>{billedLessonCount}</b></div>
                    <div>יעד מחושב<br/><b>{money(baseAnnualTarget)}</b></div>
                    <div>יעד לתשלום לאחר יתרה קודמת<br/><b>{money(netAnnualTarget)}</b></div>
                  </div>
                  {Math.abs(openingFinancialBalance) > 0.009 && (
                    <div className={openingFinancialBalance > 0 ? 'text-green-700 dark:text-green-400' : 'text-destructive'}>
                      יתרה משנה קודמת: <b>{openingFinancialBalance > 0 ? 'זכות' : 'חוב'} {money(Math.abs(openingFinancialBalance))}</b>
                    </div>
                  )}
                  <div>
                    תשלום חודשי מחושב: <b>{money(previewMonthlyAmount)}</b>
                  </div>
                </div>
              </>
            )}
            ...
            <div>
              ...
              <Textarea id="notes" value={studentForm.notes} onChange={(e) => setStudentForm({...studentForm, notes: e.target.value})} placeholder="הערות נוספות" rows={3} />
            </div>
